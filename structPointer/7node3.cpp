...
node* headNode(int val);
void print(node *current);
void delVal(node *current);

int main()
{
    node *head , *current, *tail;      // Pointer of type node 
...
current=head;
print(head);
while (current->next->val!=5)
{
    current = current->next;
}
// current=current->next;
cout<<"Deleting node with value =  "<<current->next->val<< endl;
delVal(current);
